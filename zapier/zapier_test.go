...
const (
	...
	testBaseDefaultURL = "https://api.zapier.com/v1/"
)
...
var (
	...
	// client is the Zapier client being tested.
	client *Client
...
// setup sets up a test HTTP server along with a zapier.Client that is
...
func setup() {
	...
	// zapier client configured to use test server
...
func TestNewClient(t *testing.T) {
	c := NewClient(version, clientId, nil)

	if got, want := c.BaseURL.String(), testBaseDefaultURL; got != want {
		t.Errorf("NewClient BaseURL is %v, want %v", got, want)
	}
	if got, want := c.UserAgent, userAgent; got != want {
		t.Errorf("NewClient UserAgent is %v, want %v", got, want)
	}
	if got, want := c.ClientId, clientId; got != want {
		t.Errorf("NewClient ClientId is %v, want %v", got, want)
	}
}
...
func TestNewRequest(t *testing.T) {
	...
	inURL, outURL := "foo", testBaseDefaultURL+"foo?app=myApp&client_id="+clientId
	inParams := &url.Values{"app": []string{"myApp"}}
	type T struct {
		Id int `json:"id"`
	}
	inBody, outBody := &T{Id: 3}, `{"id":3}`+"\n"
	req, _ := c.NewRequest("GET", inURL, inParams, inBody)
